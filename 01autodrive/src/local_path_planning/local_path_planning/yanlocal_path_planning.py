...
class LocalPathPlanning(Node):
    def __init__(self):
        ...
        self.previous_speed = 0.0  # 用于计算加速度的前一时刻速度
        self.previous_timestamp = 0.0  # 用于计算加速度的前一时刻时间戳

        self.ACC_speed_max = yaml_data.ACC_speed_max                             #最大跟车速度
        self.ACC_speed_min = yaml_data.ACC_speed_min                             #最小跟车速度
        self.change_line_speed = yaml_data.change_line_speed                     #绕行速度

        self.max_platoon_speed = yaml_data.max_platoon_speed                     #编队行驶最高速度
        self.safe_platoon_distance = yaml_data.safe_platoon_distance             #编队行驶安全距离

        # 加速度控制参数
        self.max_acceleration = yaml_data.get('max_acceleration', 2.0)                       #最大加速度
        self.max_deceleration = yaml_data.get('max_deceleration', 3.0)                       #最大减速度
        self.change_line_max_acceleration = yaml_data.get('change_line_max_acceleration', 1.0)     #换道时最大加速度
        self.change_line_max_deceleration = yaml_data.get('change_line_max_deceleration', 1.5)     #换道时最大减速度
        self.turning_max_acceleration = yaml_data.get('turning_max_acceleration', 1.0)             #转弯时最大加速度
        self.turning_max_deceleration = yaml_data.get('turning_max_deceleration', 2.0)             #转弯时最大减速度
    ...
    def pub_callback_local_path_planning(self):
        ...
        if self.now_idx == -1:
            now_idx = err_.argmin()
            print("!!!!!!!!!!!!!!!!!!!1now_idx=%d,"%(now_idx))
        else:
            searchN = 20
            startN = max(self.now_idx-searchN,0)
            endN = min(self.now_idx+searchN,len(err_))
            now_idx = err_[startN:endN].argmin() + startN
        ...
        # 计算加速度信息
        loc_path_acc = self.calculate_acceleration(loc_path_vel, now_ts)
    ...
    def calculate_acceleration(self, loc_path_vel, current_timestamp):
        """
        计算路径上各点的加速度
        :param loc_path_vel: 路径速度数组
        :param current_timestamp: 当前时间戳
        :return: 加速度数组
        """
        loc_path_acc = np.zeros_like(loc_path_vel)
        
        # 计算纵向加速度（基于时间的速度变化）
        if self.previous_timestamp > 0 and current_timestamp > self.previous_timestamp:
            dt = current_timestamp - self.previous_timestamp
            if dt > 0.001:  # 避免除零
                # 当前速度与前一时刻速度的加速度
                current_speed = loc_path_vel[0] if len(loc_path_vel) > 0 else 0.0
                longitudinal_acc = (current_speed - self.previous_speed) / dt
                
                # 根据当前状态限制加速度范围
                if self.is_changing_line:
                    # 换道时的加速度限制
                    longitudinal_acc = max(-self.change_line_max_deceleration, 
                                         min(self.change_line_max_acceleration, longitudinal_acc))
                elif self.is_turning:
                    # 转弯时的加速度限制
                    longitudinal_acc = max(-self.turning_max_deceleration, 
                                         min(self.turning_max_acceleration, longitudinal_acc))
                else:
                    # 正常行驶时的加速度限制
                    longitudinal_acc = max(-self.max_deceleration, 
                                         min(self.max_acceleration, longitudinal_acc))
            else:
        ...
        # 计算路径上各点的加速度（基于速度变化）
        for i in range(len(loc_path_vel)):
            if i == 0:
                # 第一个点使用纵向加速度
                loc_path_acc[i] = longitudinal_acc
            elif i == len(loc_path_vel) - 1:
                # 最后一个点使用前一个点的加速度
                loc_path_acc[i] = loc_path_acc[i-1]
            else:
                # 中间点基于前后速度差计算加速度
                # 假设路径点间距约为0.5米，车速为v m/s时，时间间隔约为0.5/v秒
                if loc_path_vel[i] > 0.1:  # 避免除零
                    dt_estimated = 0.5 / loc_path_vel[i]  # 估计时间间隔
                    speed_diff = loc_path_vel[i+1] - loc_path_vel[i-1]
                    acc = speed_diff / (2 * dt_estimated)
                    
                    # 根据不同情况调整加速度
                    if self.is_changing_line:
                        # 换道时的加速度限制
                        acc = max(-self.change_line_max_deceleration, 
                                min(self.change_line_max_acceleration, acc))
                    elif self.is_turning:
                        # 转弯时的加速度限制
                        acc = max(-self.turning_max_deceleration, 
                                min(self.turning_max_acceleration, acc))
                    else:
                        # 正常行驶时的加速度限制
                        acc = max(-self.max_deceleration, 
                                min(self.max_acceleration, acc))
        ...
        # 平滑加速度变化，避免突变
        if len(loc_path_acc) > 1:
            for i in range(1, len(loc_path_acc)):
                # 限制相邻点之间的加速度变化率
                max_acc_change = 1.0  # 最大加速度变化率 m/s²
                acc_change = loc_path_acc[i] - loc_path_acc[i-1]
                if abs(acc_change) > max_acc_change:
                    loc_path_acc[i] = loc_path_acc[i-1] + np.sign(acc_change) * max_acc_change
        
        # 更新历史数据
        self.previous_speed = loc_path_vel[0] if len(loc_path_vel) > 0 else 0.0
        self.previous_timestamp = current_timestamp
        
        print(f"Calculated acceleration: {loc_path_acc[0]:.3f} m/s² (状态: 换道={self.is_changing_line}, 转弯={self.is_turning})")
