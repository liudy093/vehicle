...
class parsingNet(torch.nn.Module):
    def __init__(self, size=(288, 800), pretrained=True, backbone='50', cls_dim=(37, 10, 4), use_aux=False):
        ...
        self.model = resnet(backbone, pretrained=pretrained)  # resnet18 的网络结构
    ...
    def forward(self, x):
        ...
        # -> n 2048
        fea = self.model(x)  # 1, 3 , 288, 800 -> 1, 512, 9, 25
        if self.use_aux:
        ...
        fea = self.pool(fea).view(-1, 1800)  # 1, 512, 9, 25 -> 1, 1800 其实就是1X1的卷积核进行压缩

        group_cls = self.cls(fea).view(-1, *self.cls_dim)  # 1, 512, 9, 25 -> 1, 201, 18, 4
