...
# #####################
    ...
    def sub_callback_local_path_planning(self, msgLocalPathPlanning:LocalPathPlanningInterface):
        ...
        # 使用范围判断，只要有足够多的点就认为是有效的轨迹
        # 将原来的固定500点判断改为至少有50个点（可以根据需要调整这个阈值）
        if len(msgLocalPathPlanning.latitude) >= 50:
            ...
            # 确保前瞻索引不超过轨迹长度
            actual_advance = min(advanceDistance, len(msgLocalPathPlanning.latitude)-1)
            
            self.refLatitude = msgLocalPathPlanning.latitude[actual_advance]
            self.refLongitude = msgLocalPathPlanning.longitude[actual_advance]
            self.refYaw = msgLocalPathPlanning.angle[actual_advance]/180*PI

            self.turn_signals = msgLocalPathPlanning.turn_signals

            # 判断是否到达终点
            # 检查轨迹是否闭合（首尾点接近）且车辆已停止
            if len(msgLocalPathPlanning.latitude) > 1:
                if abs(msgLocalPathPlanning.latitude[0] - msgLocalPathPlanning.latitude[-1]) < 1e-8 and \
                    abs(msgLocalPathPlanning.longitude[0] - msgLocalPathPlanning.longitude[-1]) < 1e-8 and \
                    self.nowSpeed < 0.001:
                    self.arrived = True
                else:
                    self.arrived = False
