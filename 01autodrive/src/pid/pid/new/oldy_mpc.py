...
class YMpc(Node):
    ...
    def __init__(self, name):
        ...
        self._control_cycle_count = 0  # 控制周期计数器
        ...
        root_path = os.getcwd() + "/src/pid/y_mpc/"  # 根目录
        # ======================================================== #
        # 从config文件中读取超参数
        ...
        self.declare_parameter('pid_kp', 0.4)    # 提高比例增益
        self.declare_parameter('pid_ki', 0.01)   # 适度提高积分增益
    ...
    def mpc_callback(self):
        ...
        # 平稳启动状态机（针对低速环境优化）
        if startup_time < 6.0:  # 前6秒为启动阶段（低速环境）
                ...
                if hasattr(self, 'get_virtual_leader'):
                    ...
                    if ref_arc_lengths is None:
                        ...
                        target_acceleration, control_strategy, mpc_status = self.normal_operation_control(
                            ref_arc_lengths, ref_velocities, ref_accelerations
                        )
                else:
                    # MPC系统未初始化，使用备用速度控制
        ...
        # =============== 更新消息并发布 ===============
        msgYmpc.throttle = control_cmd['throttle']
        msgYmpc.brake = control_cmd['brake']
    ...
    def smooth_startup_control(self, startup_time):
        ...
        # 阶段2: 缓慢启动阶段 (1.5-4秒) - 适合低速环境的启动
            ...
            if self.nowSpeed > 2.5:  # 启动阶段限制在2.5m/s
                target_acceleration = min(target_acceleration, 0.0)
        ...
        # 阶段3: 过渡到正常控制 (4-6秒) - 缩短过渡时间
            ...
            # 逐渐过渡到参考速度控制
            progress = (startup_time - 4.0) / 2.0  # 0到1的进度，2秒完成过渡
