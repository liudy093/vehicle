...
class YMpc(Node):
    ...
    def __init__(self, name):
        ...
        self.prev_velocity_for_acc = None
        ...
        self.virtual_leader_log_path = os.path.join(self.data_dir, 'virtual_leader_state.csv')
        self.vehicle_state_log_path = os.path.join(self.data_dir, 'vehicle_tracking_state.csv')
        self.log_step = 0
        self.latest_virtual_leader = ([], [], [])
    ...
    def mpc_callback(self):
        ...
        if hasattr(self, 'log_latest_states'):
    ...
    def sub_callback_local_path_planning(self, msgLocalPathPlanning:LocalPathPlanningInterface):
        """订阅局部路径规划数据"""
        self.last_path_planning_msg = msgLocalPathPlanning

        if msgLocalPathPlanning.latitude is None or len(msgLocalPathPlanning.latitude) == 0:
            self.trajectory_state = False
            return

        if hasattr(msgLocalPathPlanning, 'speed') and len(msgLocalPathPlanning.speed) > 0:
            self.trajectory_speeds = list(msgLocalPathPlanning.speed)
            self.refSpeed = msgLocalPathPlanning.speed[0]
        else:
            self.trajectory_speeds = []

        acceleration_profile = getattr(msgLocalPathPlanning, 'acceleration', [])
        if acceleration_profile and len(acceleration_profile) == len(self.trajectory_speeds):
            self.trajectory_accelerations = list(acceleration_profile)
        elif self.trajectory_speeds:
            self.trajectory_accelerations = self.compute_acceleration_profile(self.trajectory_speeds)
        else:
            self.trajectory_accelerations = []
    ...
    def get_virtual_leader(self):
        ...
        if self.last_path_planning_msg is None:
            self.latest_virtual_leader = ([], [], [])
            print("Warning: No path planning data available for virtual leader generation")
        ...
        raw_speed_profile = getattr(self.last_path_planning_msg, 'speed', [])
        if self.trajectory_speeds:
            speed_profile = self.trajectory_speeds
        else:
            try:
                speed_profile = list(raw_speed_profile)
            except TypeError:
                speed_profile = []

        raw_accel_profile = getattr(self.last_path_planning_msg, 'acceleration', [])
        if self.trajectory_accelerations:
            accel_profile = self.trajectory_accelerations
        else:
            try:
                accel_profile = list(raw_accel_profile)
            except TypeError:
                accel_profile = []
            ...
            try:
                ...
                if trajectory_idx is not None and trajectory_idx < len(self.last_path_planning_msg.latitude):
                    ...
                    if self.validate_gps_coordinates(traj_lat, traj_lon):
                        # 速度参考生成
                        if speed_profile and trajectory_idx < len(speed_profile):
                            base_ref_speed = speed_profile[trajectory_idx]
                        else:
                        ...
                        previous_speed = ref_velocities[-1] if ref_velocities else self.current_velocity
                        ref_speed = max(0.0, ref_speed)
                        ref_velocities.append(ref_speed)

                        accel_from_path = None
                        if accel_profile and trajectory_idx < len(accel_profile):
                            accel_from_path = accel_profile[trajectory_idx]

                        computed_accel = (ref_speed - previous_speed) / dt if dt > 0 else 0.0
                        if accel_from_path is not None:
                            ref_accel = 0.5 * accel_from_path + 0.5 * computed_accel
                        else:
                            ref_accel = computed_accel
                        ref_accel = max(-2.5, min(2.5, ref_accel))
        ...
        if ref_arc_lengths:
            ...
            self.refArcLength = ref_arc_lengths[0]
            self.refSpeed = ref_velocities[0]
            self.refAcceleration = ref_accelerations[0] if ref_accelerations else 0.0
        else:
            self.refArcLength = getattr(self, 'current_trajectory_position', 0.0)
            self.refAcceleration = 0.0

        self.latest_virtual_leader = (ref_arc_lengths, ref_velocities, ref_accelerations)
    ...
    def update_vehicle_state(self):
        ...
        try:
            ...
            if self.prev_velocity_for_acc is None:
                self.nowAcceleration = 0.0
            else:
                if self.dt > 0:
                    self.nowAcceleration = (self.current_velocity - self.prev_velocity_for_acc) / self.dt
                else:
                    self.nowAcceleration = 0.0
            self.prev_velocity_for_acc = self.current_velocity
    ...
    def _initialize_logging_files(self):
        """写入数据记录文件头"""
        try:
            with open(self.virtual_leader_log_path, 'w', newline='', encoding='utf-8') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow(['time_s', 'arc_length_m', 'speed_mps', 'acceleration_mps2'])
            with open(self.vehicle_state_log_path, 'w', newline='', encoding='utf-8') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow([
                    'time_s',
                    'vehicle_arc_length_m',
                    'vehicle_speed_mps',
                    'vehicle_acceleration_mps2',
                    'ref_arc_length_m',
                    'ref_speed_mps',
                    'ref_acceleration_mps2',
                    'speed_error_mps'
                ])
        except Exception as e:
            print(f"Error initializing log files: {e}")

    def compute_acceleration_profile(self, speed_profile):
        """基于速度序列估算加速度"""
        ...
        for i in range(1, len(speed_profile)):
            ...
            curr_speed = speed_profile[i]
            accel = (curr_speed - prev_speed) / self.dt if self.dt > 0 else 0.0
            accelerations.append(accel)
        return accelerations

    def log_latest_states(self):
        """记录当前虚拟领车和车辆状态"""
        leader_arc, leader_speed, leader_acc = 0.0, 0.0, 0.0
        if self.latest_virtual_leader and self.latest_virtual_leader[0]:
            leader_arc = self.latest_virtual_leader[0][0]
            leader_speed = self.latest_virtual_leader[1][0]
            leader_acc = self.latest_virtual_leader[2][0] if self.latest_virtual_leader[2] else 0.0

        # 若没有有效参考，则不记录，避免重复留空行
        if not self.latest_virtual_leader or not self.latest_virtual_leader[0]:
            return

        time_value = self.log_step * self.dt
        time_str = f"{time_value:.1f}"

        vehicle_arc = getattr(self, 'nowArcLength', 0.0)
        vehicle_speed = getattr(self, 'current_velocity', self.nowSpeed)
        vehicle_acc = getattr(self, 'nowAcceleration', 0.0)
        speed_error = vehicle_speed - leader_speed

        try:
            with open(self.virtual_leader_log_path, 'a', newline='', encoding='utf-8') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow([time_str, leader_arc, leader_speed, leader_acc])

            with open(self.vehicle_state_log_path, 'a', newline='', encoding='utf-8') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow([
                    time_str,
                    vehicle_arc,
            ...
            print(f"Error logging state data: {e}")
