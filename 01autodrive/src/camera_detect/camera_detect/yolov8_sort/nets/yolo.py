...
def fuse_conv_and_bn(conv, bn):
    ...
    fusion_weight = torch.mm(w_bn, w_conv)
    if torch.isnan(fusion_weight).any():
        print("break")
        return -1
    fusedconv.weight.copy_(fusion_weight.view(fusedconv.weight.shape))
    ...
    def fuse(self):
        ...
        for m in self.modules():
            if type(m) is Conv and hasattr(m, 'bn'):
                fuse_layer = fuse_conv_and_bn(m.conv, m.bn)
                if isinstance(fuse_layer, int):
                    break
                m.conv = fuse_layer  # update conv
