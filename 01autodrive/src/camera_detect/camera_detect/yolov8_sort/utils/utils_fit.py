...
def fit_one_epoch(model_train, model, ema, yolo_loss, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, fp16, scaler, save_period, save_dir, local_rank=0):
    ...
    if local_rank == 0:
        print('Start Train')
        train_pbar = tqdm(total=epoch_step,desc=f'Epoch {epoch + 1}/{Epoch}',postfix=dict,mininterval=0.3)
    model_train.train()
    for iteration, batch in enumerate(gen):
        ...
        if local_rank == 0:
            train_pbar.set_postfix(**{'loss'  : loss_value.item(), 'lr' : get_lr(optimizer)})
            train_pbar.update(1)
    ...
    if local_rank == 0:
        ...
        val_pbar = tqdm(total=epoch_step_val, desc=f'Epoch {epoch + 1}/{Epoch}',postfix=dict,mininterval=0.3)
    ...
    for iteration, batch in enumerate(gen_val):
        ...
        if local_rank == 0:
            val_pbar.set_postfix(**{'val_loss': loss_value.item()})
            val_pbar.update(1)
