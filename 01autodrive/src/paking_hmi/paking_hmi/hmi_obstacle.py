...
def world_to_canvas(world_auto_pose, canvas_auto_pose, world_path_pose, scale):
    """
    @param world_auto_pose 世界坐标系下自动驾驶车辆位置和姿态信息[x y yaw] yaw为弧度
    @param canvas_auto_pose 画布坐标系下自动驾驶车辆位置信息[x y]
    @param world_path_pose 世界坐标系下路径位置信息[x y]
    @param scale 世界坐标系到画布坐标系的距离扩大系数 [scale_x scale_y]
    @return canvas_path_pose 返回画布坐标系下参考路径位置
    """
...
class Hmi(Node):
    def __init__(self, name):
        ...
        self.subObstacle = self.create_subscription(GpsInterface, "/obstacle/gps_data",self.obstaclePose_callback, 10)  # 后面删除
    ...
    def sub_callback_car_ori(self, msgCarOri:CarOriInterface):
        ...
        self.text_data[3,3] = self.CanData.steerangle
        
    def obstaclePose_callback(self, msg:GpsInterface):
        """
        障碍车位置回调函数，可以删除
        """
        self.obstaclePose = msg
    ...
    def hmi_callback(self):
        ...
        if self.autoPose is not None:
            ...
            if self.planPath is not None:                       
                ...
                if self.obstaclePose is not None:  # 后面删除
                    obstacle_pos_ = np.concatenate([np.array(self.obstaclePose.x).reshape(-1,1), np.array(self.obstaclePose.y).reshape(-1,1)], axis=1)
                    canvas_obstacle_pose_ = world_to_canvas(world_auto_pose_, canvas_auto_pose, obstacle_pos_, scale)
                    cv2.circle(canvas, (canvas_obstacle_pose_[0,0], canvas_obstacle_pose_[0,1]), 5, (0,0,0), -1)
