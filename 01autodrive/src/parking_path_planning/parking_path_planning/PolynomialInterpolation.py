...
if __name__ == "__main__":
    ...
    q_given = np.array([[0, 21, 14.2, 15.7],
                        [0, 0, 0, 0],
                        [0, 0, 0, 0]]).transpose()
    t_given = np.array([0, 46, 63, 70]).transpose()
    # time for interpolation
    t = np.linspace(t_given[0], t_given[-1], 70*20)
    ...
    ref_vel = np.loadtxt(fname='velocity_data.txt', dtype=np.float64, encoding='utf-8')
    vel = list(vel)
    ...
    plt.plot(t_given, q_given[:, 0], 'ro')
    plt.plot(t, polynomial5_trajectory[:,0], 'r')
    plt.grid(False)
    # plt.title('Polynomial of degree 5 interpolation')
    plt.xlabel('时间$(s)$',font)
    plt.ylabel('行进距离$(m)$',font)
    plt.xlim(t_given[0]-1, t_given[-1]+1)
    # plt.ylim(min(q_given[:,0]) - 1, max(q_given[:,0]) + 1)
    plt.ylim(-1, 25)
    ...
    plt.subplot(3,1,2)
    plt.plot(t_given, q_given[:, 1], 'ro')
    plt.plot(t, polynomial5_trajectory[:,1], 'b')
    plt.grid(False)
    plt.xlabel('时间$(s)$',font)
    plt.ylabel('速度$(m/s)$',font)
    plt.xlim(t_given[0]-1, t_given[-1]+1)
    plt.ylim(-1, 1)

    plt.subplot(3,1,3)
    plt.plot(t_given, q_given[:, 2], 'ro')
    plt.plot(t, polynomial5_trajectory[:,2], 'peru')
    plt.grid(False)
    plt.xlabel('时间$(s)$',font)
    plt.ylabel('加速度$(m/s^2)$',font)
    plt.xlim(t_given[0]-1, t_given[-1]+1)
    plt.ylim(-0.2, 0.2)
