...
ros_node_name = "left_lidar_obstacle"
...
obstacle_path = os.getcwd() + '/src/left_lidar_obstacle/left_lidar_obstacle'
...
class LidarObstacle(Node):
    def __init__(self, name):
        ...
        # ======================================================== #
        # 从config文件中读取超参数
        root_path = os.getcwd() + '/src/left_lidar_obstacle/left_lidar_obstacle/'  # 根目录
        ...
        # ======================================================== #
        # 订阅器
        self.subLidarOri_rslidar = self.create_subscription(PointCloud2, '/left/rslidar_points',
                                                            self.sub_callback_lidar_rslidar, 10)

        # 监听器
        # ======================================================== #
        self.PubLeftLidarState = self.create_publisher(Bool, 'LeftLidarState', 10)
        self.left_msg_recieve_flag = False  # 默认状态错误，需要在代码中不断的改为正确
        self.timerRidarOri = self.create_timer(1, self.LefttLidarState_monitor)
        # ======================================================== #
        ...
        # 发布器
        # 障碍物数据
        self.pubLidarObstacle = self.create_publisher(LidarObstacleInterface, 'left_lidar_obstacle_data', 10)
        # grid可视化
        if self.config.vis_publisher.obstacle_grid:
            self.obstacle_grid = self.create_publisher(GridCells, 'left_obstacle_grid', 10)
        # 可视化旋转之后的全部点云
        if self.config.vis_publisher.all_points:
            self.pubLidar_all_points = self.create_publisher(PointCloud2, 'left_point_after_trans', 10)
            
    def LefttLidarState_monitor(self):
        """
        毫米波状态监听器
        """
        msg = Bool()
        if self.left_msg_recieve_flag  == True:
            msg.data = True
            self.PubLeftLidarState.publish(msg)
            self.left_msg_recieve_flag = False
        else:
    ...
    def publish_lidar_obstacle(self, obstacle_np):
        ...
        # state monitor
        # ======================================================== #
        self.left_msg_recieve_flag = True
        msg = Bool()
        msg.data = True
        self.PubLeftLidarState.publish(msg)
        # ======================================================== #
...
def main():
    rclpy.init()
    rosNode = LidarObstacle(name='left_lidar_obstacle')
    rclpy.spin(rosNode)
    rclpy.shutdown()
